"""
Coordinate Projector (5D)
위상 → 5D 좌표/각도 투영 (관측자 책임)

이 모듈은 Grid 5D Engine의 내부 위상을 외부 5D 좌표/각도로 투영합니다.
Grid 5D Engine은 내부 위상(phi_x, phi_y, phi_z, phi_a, phi_b)만 관리하고, 좌표/각도 투영은 이 모듈(관측자)의 책임입니다.

5D 확장 (5축 CNC):
    - 2D: 위상 (φx, φy) → 좌표 (x, y)
    - 3D: 위상 (φx, φy, φz) → 좌표 (x, y, z)
    - 4D: 위상 (φx, φy, φz, φw) → 좌표 (x, y, z, w)
    - 5D: 위상 (φx, φy, φz, φa, φb) → 좌표/각도 (x, y, z, θa, θb) ✨ NEW

아키텍처 원칙:
    책임 분리(Separation of Concerns):
        - Grid 5D Engine: 내부 위상 상태만 관리 (φx, φy, φz, φa, φb)
        - Coordinate5DProjector: 위상 → 5D 좌표/각도 변환 (관측자 패턴)
...
수학적 배경:
    Grid 5D Engine은 토러스(Torus) T⁵ = S¹ × S¹ × S¹ × S¹ × S¹에서 동작합니다.
...
import math
from .types_5d import Grid5DState
from .config_5d import Grid5DConfig


class Coordinate5DProjector:
    """
    5D 좌표/각도 투영기 (Observer)
    
    Grid 5D Engine의 내부 위상을 외부 5D 좌표/각도로 투영합니다.
    Grid 5D Engine은 위상만 유지하고, 좌표/각도 계산은 이 클래스가 담당합니다.
    
    5D 확장 (5축 CNC):
        - 2D: (φx, φy) → (x, y)
        - 3D: (φx, φy, φz) → (x, y, z)
        - 4D: (φx, φy, φz, φw) → (x, y, z, w)
        - 5D: (φx, φy, φz, φa, φb) → (x, y, z, θa, θb) ✨ NEW
    ...
    def __init__(self, config: Grid5DConfig):
        """
        Args:
            config: Grid 5D Engine 설정
        """
        self.config = config
    ...
    def phase_to_coordinate(
        ...
        내부 위상(Phase)을 외부 5D 좌표/각도(Coordinate/Angle)로 변환합니다.
        Grid 5D Engine의 내부 상태를 외부 세계의 5D 위치/각도로 투영합니다.
        
        수학적 배경:
            Grid 5D Engine은 토러스 T⁵ = S¹ × S¹ × S¹ × S¹ × S¹에서 동작합니다.
        ...
        # ⚠️ 단위: phi_a, phi_b는 [rad] (내부 단위)
        #          theta_a, theta_b는 [deg] (출력 단위)
    ...
    def coordinate_to_phase(
        ...
        외부 5D 좌표/각도(Coordinate/Angle)를 내부 위상(Phase)으로 변환합니다.
        외부 세계의 5D 위치/각도를 Grid 5D Engine의 내부 상태로 투영합니다.
        
        수학적 배경:
            phase_to_coordinate_5d()의 역변환입니다.
        ...
        # 각도 변환: φa = θa · (π / 180°)
        # ⚠️ 단위: theta_a, theta_b는 [deg] (입력 단위)
        #          phi_a, phi_b는 [rad] (내부 단위)
        #          math.radians() 사용하여 명확한 변환
        import math
        phi_a = math.radians(theta_a)  # deg → rad (명확한 변환)
        phi_b = math.radians(theta_b)  # deg → rad (명확한 변환)
        
        return phi_x, phi_y, phi_z, phi_a, phi_b
    
    def project_state(self, state: Grid5DState) -> Tuple[float, float, float, float, float]:
        """
        상태의 위상을 5D 좌표/각도로 투영
        
        Args:
            state: Grid 5D 상태 (위상 포함)
        
        Returns:
            (x, y, z, theta_a, theta_b) 좌표/각도
                - x, y, z: 위치 [m]
...
def create_projector_5d(config: Grid5DConfig) -> Coordinate5DProjector:
    """
    Coordinate5DProjector 생성
    
    Args:
        config: Grid 5D Engine 설정
    
    Returns:
        Coordinate5DProjector 인스턴스
    """
    return Coordinate5DProjector(config)
