"""
Coordinate Projector (7D)
위상 → 7D 좌표/각도 투영 (관측자 책임)

이 모듈은 Grid 7D Engine의 내부 위상을 외부 7D 좌표/각도로 투영합니다.
Grid 7D Engine은 내부 위상(phi_x, phi_y, phi_z, phi_a, phi_b)만 관리하고, 좌표/각도 투영은 이 모듈(관측자)의 책임입니다.

7D 확장 (7축 시스템):
    - 2D: 위상 (φx, φy) → 좌표 (x, y)
    - 3D: 위상 (φx, φy, φz) → 좌표 (x, y, z)
    - 4D: 위상 (φx, φy, φz, φw) → 좌표 (x, y, z, w)
    - 7D: 위상 (φx, φy, φz, φa, φb, φc, φd) → 좌표/각도 (x, y, z, θa, θb, θc, θd) ✨ NEW

아키텍처 원칙:
    책임 분리(Separation of Concerns):
        - Grid 7D Engine: 내부 위상 상태만 관리 (φx, φy, φz, φa, φb, φc, φd)
        - Coordinate7DProjector: 위상 → 7D 좌표/각도 변환 (관측자 패턴)
...
수학적 배경:
    Grid 7D Engine은 토러스(Torus) T⁷ = S¹ × S¹ × S¹ × S¹ × S¹ × S¹ × S¹에서 동작합니다.
...
import math
from .types_7d import Grid7DState
from .config_7d import Grid7DConfig


class Coordinate7DProjector:
    """
    7D 좌표/각도 투영기 (Observer)
    
    Grid 7D Engine의 내부 위상을 외부 7D 좌표/각도로 투영합니다.
    Grid 7D Engine은 위상만 유지하고, 좌표/각도 계산은 이 클래스가 담당합니다.
    
    7D 확장 (7축 시스템):
        - 2D: (φx, φy) → (x, y)
        - 3D: (φx, φy, φz) → (x, y, z)
        - 4D: (φx, φy, φz, φw) → (x, y, z, w)
        - 7D: (φx, φy, φz, φa, φb, φc, φd) → (x, y, z, θa, θb, θc, θd) ✨ NEW
    ...
    def __init__(self, config: Grid7DConfig):
        """
        Args:
            config: Grid 7D Engine 설정
        """
        self.config = config
    ...
    def phase_to_coordinate(
        ...
        내부 위상(Phase)을 외부 7D 좌표/각도(Coordinate/Angle)로 변환합니다.
        Grid 7D Engine의 내부 상태를 외부 세계의 7D 위치/각도로 투영합니다.
        
        수학적 배경:
            Grid 7D Engine은 토러스 T⁷ = S¹ × S¹ × S¹ × S¹ × S¹ × S¹ × S¹에서 동작합니다.
        ...
        # ⚠️ 단위: phi_a, phi_b, phi_c, phi_d는 [rad] (내부 단위)
        #          theta_a, theta_b, theta_c, theta_d는 [deg] (출력 단위)
    ...
    def coordinate_to_phase(
        ...
        외부 7D 좌표/각도(Coordinate/Angle)를 내부 위상(Phase)으로 변환합니다.
        외부 세계의 7D 위치/각도를 Grid 7D Engine의 내부 상태로 투영합니다.
        
        수학적 배경:
            phase_to_coordinate_7d()의 역변환입니다.
        ...
        # 각도 변환: φa = θa · (π / 180°)
        # ⚠️ 단위: theta_a, theta_b, theta_c, theta_d는 [deg] (입력 단위)
        #          phi_a, phi_b, phi_c, phi_d는 [rad] (내부 단위)
        ...
        phi_d = math.radians(theta_d)  # deg → rad (명확한 변환)
        
        return phi_x, phi_y, phi_z, phi_a, phi_b, phi_c, phi_d
    
    def project_state(self, state: Grid7DState) -> Tuple[float, float, float, float, float]:
        """
        상태의 위상을 7D 좌표/각도로 투영
        
        Args:
            state: Grid 7D 상태 (위상 포함)
        
        Returns:
            (x, y, z, theta_a, theta_b, theta_c, theta_d) 좌표/각도
                - x, y, z: 위치 [m]
...
def create_projector_7d(config: Grid7DConfig) -> Coordinate7DProjector:
    """
    Coordinate7DProjector 생성
    
    Args:
        config: Grid 7D Engine 설정
    
    Returns:
        Coordinate7DProjector 인스턴스
    """
    return Coordinate7DProjector(config)
